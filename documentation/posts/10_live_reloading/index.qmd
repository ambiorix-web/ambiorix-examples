...
subtitle: "Manual reloading is such a pain!"
...
1. Change to your project's root dir.
   
   ```bash
   cd myproject
   ```
1. Initialize an npm project & create the `package.json` file:
...
1. You can as well add `nodemon.json`, `package-lock.json`, & `package.json` to `.gitignore` since they're just used for development purposes. I have commited them for this example just so you can see their contents.
